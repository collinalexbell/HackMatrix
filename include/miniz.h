/* miniz.c v1.16 beta r1 - public domain deflate/inflate, zlib-subset, ZIP
...
extern "C"
{
  ...
  typedef enum
  {
    ...
    MZ_ZIP_FLAG_WRITE_ZIP64 = 0x4000, /* use the zip64 file format, instead of
                                         the original zip file format */
   ...
   * position. */
  /* The archive is assumed to be archive_size bytes long. If archive_size is <
   * 0, then the entire rest of the file is assumed to contain the archive. */
  ...
  int mz_zip_locate_file(mz_zip_archive* pZip,
                         const char* pName,
                         const char* pComment,
                         mz_uint flags);
  /* Returns MZ_FALSE if the file cannot be found. */
  mz_bool mz_zip_locate_file_v2(mz_zip_archive* pZip,
                                const char* pName,
                                const char* pComment,
                                mz_uint flags,
                                mz_uint32* pIndex);
...
/* TODO */
  ...
  /* Finalizes a heap archive, returning a poiner to the heap block and its
   * size. */
