...
// Note: we do not need miniz stdio functions so can define MINIZ_NO_STDIO in
...
#define ENKI_MI_REGION_CHUNKS_NUMBER 1024
  ...
  // FreeMemoryAllocated() should still be called to free any internal
  // allocations.
  void enkiNBTInitFromMemoryUncompressed(enkiNBTDataStream* pStream_,
                                         uint8_t* pUnCompressedData_,
                                         uint32_t dataSize_);

  // Initialize stream from memory pointer to compressed content.
  // This function will allocate space for uncompressed stream and decompress it
  // with zlib. If uncompressedSizeHint_ > compressedDataSize_ it will be used
  // as the starting hint size for allocating the uncompressed size. returns 1
  // if successfull, 0 if not.
  int enkiNBTInitFromMemoryCompressed(enkiNBTDataStream* pStream_,
                                      uint8_t* pCompressedData_,
                                      uint32_t compressedDataSize_,
                                      uint32_t uncompressedSizeHint_);
  ...
  void enkiInitNBTDataStreamForChunk(enkiRegionFile regionFile_,
                                     int32_t chunkNr_,
                                     enkiNBTDataStream* pStream_);

  int32_t enkiGetTimestampForChunk(enkiRegionFile regionFile_,
                                   int32_t chunkNr_);
...
// World height changes (1.17 21w06a) increase num sections to a potential 256
// (-128 to 127 as Y uses signed byte)
...
#define ENKI_MI_SIZE_SECTIONS 16
  ...
  typedef struct enkiMINamespaceAndBlockID_s
  {
    ...
    uint8_t blockID;   // block ID returned by enkiGetChunkSectionVoxel and
                       // enkiGetChunkSectionVoxelData
...
#define ENKI_MI_MAX_PROPERTIES 6
  ...
  typedef struct enkiChunkSectionPalette_s
  {
    ...
    int32_t*
      pDefaultBlockIndex; // lookup index into the default
                          // enkiMINamespaceAndBlockIDTable - these values may
                          // change with versions of enkiMI, <0 means not found
    enkiNBTString* pNamespaceIDStrings; // e.g. "minecraft:stone"
    enkiMIProperties*
      pBlockStateProperties; // pointer to start of stream properties
  ...
  typedef struct enkiChunkBlockData_s
  {
    ...
    enkiChunkSectionPalette
      palette[ENKI_MI_NUM_SECTIONS_PER_CHUNK]; // if there is a palette[k].size,
                                               // then sections[k] represents
                                               // BlockStates
  ...
  typedef enum
  {
    ...
    enkiNBTReadChunkExFlags_NoPaletteTranslation =
      1 << 0, // when loading palette do not translate namespace strings to
              // blockID & dataValue - faster if you want to do your own
              // translation / conversion to internal data
  } enkiNBTReadChunkExFlags;

  typedef struct enkiNBTReadChunkExParams_s
  {
    int32_t
      flags; // enkiNBTReadChunkExFlags defaults to enkiNBTReadChunkExFlags_None
  } enkiNBTReadChunkExParams;

  // call enkiGetDefaultNBTReadChunkExParams to set up default parameters -
  // essential to maintain forwards compatibilty if new members are added to
  // enkiNBTReadChunkExParams
  ...
  enkiChunkBlockData enkiNBTReadChunkEx(enkiNBTDataStream* pStream_,
                                        enkiNBTReadChunkExParams params_);
  ...
  enkiMICoordinate enkiGetChunkSectionOrigin(enkiChunkBlockData* pChunk_,
                                             int32_t section_);
  ...
  uint8_t enkiGetChunkSectionVoxel(enkiChunkBlockData* pChunk_,
                                   int32_t section_,
                                   enkiMICoordinate sectionOffset_);

  uint32_t* enkiGetMineCraftPalette(); // returns a 256 array of uint32_t's in
                                       // uint8_t rgba order.

  typedef struct enkiMIVoxelData_s
  {

    uint8_t blockID;   // pre-flattening blockIDs values, as returned by
                       // enkiGetChunkSectionVoxel(), can use to index into
                       // enkiGetMineCraftPalette
    uint8_t dataValue; // pre-flattening data values, blockId::dataValue
                       // identifies block varients
    int32_t
      paletteIndex; // if >=0 index into
                    // enkiChunkBlockData.palette[section].pDefaultBlockIndex
                    // and
                    // enkiChunkBlockData.palette[section].pNamespaceIDStrings
  } enkiMIVoxelData;

  enkiMIVoxelData enkiGetChunkSectionVoxelData(enkiChunkBlockData* pChunk_,
                                               int32_t section_,
                                               enkiMICoordinate sectionOffset_);
