...
std::shared_ptr<spdlog::sinks::rotating_file_sink_mt> fileSink =
  std::make_shared<spdlog::sinks::rotating_file_sink_mt>("logs/matrix.log",
                                                         50000,
                                                         0);

void
LoggerSink::sink_it_(const spdlog::details::log_msg& msg)
{
...
void
ImGuiSink::sink_it_(const spdlog::details::log_msg& msg)
{
  ...
  formattedMsg << "[" << msg.logger_name.data() << "] " << msg.payload.data()
               << std::endl;
...
void
LoggerVector::log(std::string msg)
{
