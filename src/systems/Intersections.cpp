...
namespace systems {
void
emplaceBoundingSphere(std::shared_ptr<EntityRegistry> registry,
                      entt::entity entity)
{
  ...
  registry->emplace_or_replace<BoundingSphere>(
    entity, boundingSphere.center, boundingSphere.radius);
}

bool
intersect(const BoundingSphere& sphere,
          glm::vec3 position,
          glm::vec3 direction,
          float maxDistance)
{
...
bool
isOnFrustum(std::shared_ptr<EntityRegistry> registry,
            entt::entity entity,
...
bool
isOnOrForwardPlane(BoundingSphere* sphere, Plane plane)
{
