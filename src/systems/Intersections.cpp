                 ...
                 float maxDistance)
{
    ...
    // If the discriminant is negative, there are no real roots, so the ray misses the sphere
    ...
    // If the nearest root is within the maximum distance, the ray intersects the sphere
    if (nearestRoot >= 0.0f && nearestRoot <= maxDistance)
        return true;

    // If the nearest root is behind the ray origin or beyond the maximum distance,
    // check if the farther root is within the maximum distance
