...
GLenum
textureFormat(std::string fname)
{
...
void
setTextureParameters(bool mipmapped, bool pixelated)
{
...
void
setTextureArrayParameters(bool mipmapped, bool pixelated)
{
  ...
  if (pixelated) {
    if (mipmapped) {
      ...
      glTexParameteri(
        GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_NEAREST_MIPMAP_NEAREST);
    } else {
...
void
Texture::loadTextureData(std::string fname)
{
  unsigned char* data =
    stbi_load(fname.c_str(), &width, &height, &nrChannels, 0);
  if (data) {
...
void
Texture::loadTextureArrayData(vector<string> fnames)
{
  if (fnames.size() == 0) {
    cout << "no data" << endl;
    return;
  ...
  unsigned char* buffer =
    stbi_load(fnames[0].c_str(), &width, &height, &nrChannels, 4);
  int numMipLevels = 1;
  glTexStorage3D(
    GL_TEXTURE_2D_ARRAY, numMipLevels, GL_RGBA8, width, height, fnames.size());
...
{
  glTexImage2D(
    GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
}

void
Texture::initAndBindGlTexture(GLenum unit)
{
  glGenTextures(1, &ID);
...
void
Texture::initAndBindGlTextureArray(GLenum unit)
{
  glGenTextures(1, &ID);
  glActiveTexture(unit);
