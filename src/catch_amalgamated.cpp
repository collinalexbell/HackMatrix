  ...
  std::generate_n(
    std::back_inserter(out), resamples, [n, first, &estimator, &dist, &rng] {
      ...
      std::generate_n(std::back_inserter(resampled), n, [first, &dist, &rng] {
        return first[static_cast<std::ptrdiff_t>(dist(rng))];
      });
...
#endif
bool
directCompare(double lhs, double rhs)
{
  return lhs == rhs;
}
#if defined(__GNUC__) || defined(__clang__)
...
const char*
optimized_away_error::what() const noexcept
{
...
bool
operator==(ProcessedReporterSpec const& lhs, ProcessedReporterSpec const& rhs)
{
...
std::vector<ReporterSpec> const&
Config::getReporterSpecs() const
{
...
bool
Config::warnAboutUnmatchedTestSpecs() const
{
  return !!(m_data.warnings & WarnAbout::UnmatchedTestSpec);
}
bool
Config::zeroTestsCountAsSuccess() const
{
...
unsigned int
Config::shardCount() const
{
  return m_data.shardCount;
}
unsigned int
Config::shardIndex() const
{
  return m_data.shardIndex;
}
ColourMode
Config::defaultColourMode() const
{
...
IEventListenerPtr
createReporter(std::string const& reporterName, ReporterConfig&& config)
{
...
IEventListenerPtr
prepareReporters(Config const* config)
{
...
bool
applies(TestCaseProperties tcp)
{
...
bool
isReservedTag(StringRef tag)
{
  return parseSpecialTag(tag) == TestCaseProperties::None && tag.size() > 0 &&
         !std::isalnum(static_cast<unsigned char>(tag[0]));
}
void
enforceNotReservedTag(StringRef tag, SourceLineInfo const& _lineInfo)
{
  CATCH_ENFORCE(
    !isReservedTag(tag),
    "Tag name: ["
...
StringRef
extractFilenamePart(StringRef filename)
{
...
size_t
sizeOfExtraTags(StringRef filepath)
{
  // [.] is 3, [#] is another 3
...
void
TestCaseInfo::internalAppendTag(StringRef tagStr)
{
  backingTags += '[';
...
bool
operator<(TestCaseInfo const& lhs, TestCaseInfo const& rhs)
{
...
TestCaseInfo const&
TestCaseHandle::getTestCaseInfo() const
{
  return *m_info;
}
...
TestSpec::getInvalidSpecs() const
{
...
std::string
convertIntoString(StringRef string, bool escape_invisibles)
{
  std::string ret;
...
std::string
convertIntoString(StringRef string)
{
...
ColourMode
ReporterConfig::colourMode() const
{
  return m_colourMode;
}
...
void
AssertionHandler::handleMessage(ResultWas::OfType resultType, StringRef message)
{
...
bool
CaseInsensitiveLess::operator()(StringRef lhs, StringRef rhs) const
{
...
bool
CaseInsensitiveEqualTo::operator()(StringRef lhs, StringRef rhs) const
{
  return std::equal(
    lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), [](char l, char r) {
      return toLower(l) == toLower(r);
...
std::string
normaliseOpt(std::string const& optName)
{
#ifdef CATCH_PLATFORM_WINDOWS
  if (optName[0] == '/')
...
size_t
ParserBase::cardinality() const
{
  return 1;
}

InternalParseResult
ParserBase::parse(Args const& args) const
{
...
ParserResult
BoundFlagRef::setFlag(bool flag)
{
  m_ref = flag;
  return ParserResult::ok(ParseResultType::Matched);
}
...
bool
BoundRef::isContainer() const
{
  return false;
}

bool
BoundRef::isFlag() const
{
  return false;
}

bool
BoundFlagRefBase::isFlag() const
{
  return true;
}
...
std::vector<Detail::HelpColumns>
Opt::getHelpColumns() const
{
...
bool
Opt::isMatch(std::string const& optToken) const
{
...
ParserResult
ExeName::set(std::string const& newName)
{
...
void
Parser::writeToStream(std::ostream& os) const
{
...
makeCommandLineParser(ConfigData& config)
{
  ...
  auto const setShardIndex = [&](std::string const& shardIndex) {
    CATCH_TRY
    {
      ...
      if (parsedIndex < 0) {
        return ParserResult::runtimeError(
          "Shard index must be a non-negative number");
      }
...
ColourImpl::guardColour(Colour::Code colourCode)
{
  return ColourGuard(colourCode, this);
}

void
ColourImpl::ColourGuard::engageImpl(std::ostream& stream)
{
...
makeColourImpl(ColourMode implSelection, IStream* stream)
{
...
bool
isColourImplAvailable(ColourMode colourSelection)
{
...
void
FatalConditionHandler::disengage_platform() noexcept
{
}
...
static void
restorePreviousSignalHandlers() noexcept
{
...
auto
makeStream(std::string const& filename) -> Detail::unique_ptr<IStream>
{
...
void
listTests(IEventListener& reporter, IConfig const& config)
{
...
void
listTags(IEventListener& reporter, IConfig const& config)
{
  auto const& testSpec = config.testSpec();
...
void
listReporters(IEventListener& reporter)
{
...
void
listListeners(IEventListener& reporter)
{
...
void
TagInfo::add(StringRef spelling)
{
  ++count;
...
bool
list(IEventListener& reporter, Config const& config)
{
  bool listed = false;
  if (config.listTests()) {
    listed = true;
...
std::uint32_t
generateRandomSeed(GenerateFrom from)
{
  switch (from) {
...
IEventListenerPtr
ReporterRegistry::create(std::string const& name, ReporterConfig&& config) const
{
...
void
ReporterRegistry::registerListener(
...
kvPair
splitKVPair(StringRef kvString)
{
...
Optional<ColourMode>
stringToColourMode(StringRef colourMode)
{
...
bool
operator==(ReporterSpec const& lhs, ReporterSpec const& rhs)
{
...
std::string
ReusableStringStream::str() const
{
  return static_cast<std::ostringstream*>(m_oss)->str();
}

void
ReusableStringStream::str(std::string const& str)
{
  ...
  , IGeneratorTracker
{
    ...
    TestCaseTracking::NameAndLocation const& nameAndLocation)
  {
    ...
    if (currentTracker.nameAndLocation() == nameAndLocation) {
      ...
      auto newTracker = Catch::Detail::make_unique<GeneratorTracker>(
...
Totals
RunContext::runTest(TestCaseHandle const& testCase)
{
  ...
  do {
    ...
    auto statsForOneRun = TestCaseStats(
      testInfo, singleRunTotals, oneRunCout, oneRunCerr, aborting());
...
void
RunContext::benchmarkPreparing(StringRef name)
{
...
void
RunContext::benchmarkFailed(StringRef error)
{
...
bool
startsWith(StringRef s, char prefix)
{
...
{
  std::transform(
    s.begin(), s.end(), s.begin(), [](char c) { return toLower(c); });
}
std::string
...
auto
StringRef::operator==(StringRef other) const noexcept -> bool
{
...
bool
StringRef::operator<(StringRef rhs) const noexcept
{
...
int
StringRef::compare(StringRef rhs) const
{
...
auto
operator<<(std::ostream& os, StringRef str) -> std::ostream&
{
  return os.write(str.data(), static_cast<std::streamsize>(str.size()));
}

std::string
operator+(StringRef lhs, StringRef rhs)
{
  std::string ret;
...
auto
operator+=(std::string& lhs, StringRef rhs) -> std::string&
{
...
bool
isThrowSafe(TestCaseHandle const& testCase, IConfig const& config)
{
  return !testCase.getTestCaseInfo().throws() || config.allowThrows();
}

bool
matchTest(TestCaseHandle const& testCase,
...
std::vector<TestCaseInfo*> const&
TestRegistry::getAllInfos() const
{
...
void
ITracker::markAsNeedingAnotherRun()
{
  m_runState = NeedsAnotherRun;
}

void
ITracker::addChild(ITrackerPtr&& child)
{
...
bool
ITracker::isSectionTracker() const
{
  return false;
}
bool
ITracker::isGeneratorTracker() const
{
  return false;
}

bool
ITracker::isSuccessfullyCompleted() const
{
  return m_runState == CompletedSuccessfully;
}

bool
ITracker::isOpen() const
{
  return m_runState != NotStarted && !isComplete();
}

bool
ITracker::hasStarted() const
{
...
void
SectionTracker::addNextFilters(std::vector<StringRef> const& filters)
{
  if (filters.size() > 1)
    m_filters.insert(m_filters.end(), filters.begin() + 1, filters.end());
}

std::vector<StringRef> const&
...
namespace {
StringRef
extractClassName(StringRef classOrMethodName)
{
...
bool
isBoundary(std::string const& line, size_t at)
{
  assert(at > 0);
  assert(at <= line.size());
...
void
Column::const_iterator::calcLength()
{
  m_addHyphen = false;
...
size_t
Column::const_iterator::indentSize() const
{
  auto initial =
...
std::string
Column::const_iterator::operator*() const
{
...
Column::const_iterator
Column::const_iterator::operator++(int)
{
...
std::string
Columns::iterator::operator*() const
{
...
XmlWriter::ScopedElement&
XmlWriter::ScopedElement::writeAttribute(StringRef name, StringRef attribute)
{
...
XmlWriter&
XmlWriter::writeText(StringRef text, XmlFormatting fmt)
{
...
XmlWriter&
XmlWriter::writeComment(StringRef text, XmlFormatting fmt)
{
  ensureTagClosed();
  if (shouldIndent(fmt)) {
    m_os << m_indent;
...
void
XmlWriter::writeStylesheetRef(StringRef url)
{
...
bool
StringEqualsMatcher::match(std::string const& source) const
{
...
bool
StringContainsMatcher::match(std::string const& source) const
{
...
StringEqualsMatcher
Equals(std::string const& str, CaseSensitive caseSensitivity)
{
  return StringEqualsMatcher(CasedString(str, caseSensitivity));
}
StringContainsMatcher
ContainsSubstring(std::string const& str, CaseSensitive caseSensitivity)
{
  return StringContainsMatcher(CasedString(str, caseSensitivity));
}
EndsWithMatcher
EndsWith(std::string const& str, CaseSensitive caseSensitivity)
{
  return EndsWithMatcher(CasedString(str, caseSensitivity));
}
StartsWithMatcher
StartsWith(std::string const& str, CaseSensitive caseSensitivity)
{
  return StartsWithMatcher(CasedString(str, caseSensitivity));
}

RegexMatcher
Matches(std::string const& regex, CaseSensitive caseSensitivity)
{
...
std::string
IsEmptyMatcher::describe() const
{
  return "is empty";
}

std::string
HasSizeMatcher::describe() const
{
...
void
AutomakeReporter::testCaseEnded(TestCaseStats const& _testCaseStats)
{
...
void
AutomakeReporter::skipTest(TestCaseInfo const& testInfo)
{
...
namespace {
void
listTestNamesOnly(std::ostream& out, std::vector<TestCaseHandle> const& tests)
{
  for (auto const& test : tests) {
...
std::string
serializeFilters(std::vector<std::string> const& filters)
{
...
void
defaultListListeners(std::ostream& out,
...
void
defaultListTags(std::ostream& out,
...
void
EventListenerBase::fatalErrorEncountered(StringRef)
{
}

void
EventListenerBase::benchmarkPreparing(StringRef)
{
}
void
EventListenerBase::benchmarkStarting(BenchmarkInfo const&)
{
}
void
EventListenerBase::benchmarkEnded(BenchmarkStats<> const&)
{
}
void
EventListenerBase::benchmarkFailed(StringRef)
{
}

void
EventListenerBase::assertionStarting(AssertionInfo const&)
{
}

void
EventListenerBase::assertionEnded(AssertionStats const&)
{
}
void
...
void
EventListenerBase::listListeners(std::vector<ListenerDescription> const&)
{
}
void
EventListenerBase::listTests(std::vector<TestCaseHandle> const&)
{
}
void
EventListenerBase::listTags(std::vector<TagInfo> const&)
{
}
void
EventListenerBase::noMatchingTestCases(StringRef)
{
}
void
EventListenerBase::reportInvalidTestSpec(StringRef)
{
}
void
EventListenerBase::testRunStarting(TestRunInfo const&)
{
}
void
EventListenerBase::testCaseStarting(TestCaseInfo const&)
{
}
void
EventListenerBase::testCasePartialStarting(TestCaseInfo const&, uint64_t)
{
}
void
EventListenerBase::sectionStarting(SectionInfo const&)
{
}
void
EventListenerBase::sectionEnded(SectionStats const&)
{
}
void
EventListenerBase::testCasePartialEnded(TestCaseStats const&, uint64_t)
{
}
void
EventListenerBase::testCaseEnded(TestCaseStats const&)
{
}
void
EventListenerBase::testRunEnded(TestRunStats const&)
{
}
void
EventListenerBase::skipTest(TestCaseInfo const&)
{
}
} // namespace Catch
...
void
ReporterBase::listTests(std::vector<TestCaseHandle> const& tests)
{
...
void
ReporterBase::listTags(std::vector<TagInfo> const& tags)
{
...
constexpr Catch::StringRef
bothOrAll(std::uint64_t count)
{
  switch (count) {
    case 1:
...
void
printTotals(std::ostream& out, const Totals& totals, ColourImpl* colourImpl)
{
  if (totals.testCases.total() == 0) {
    ...
    out << colourImpl->guardColour(Colour::ResultError) << "Failed "
        << pluralise(totals.testCases.failed, "test case"_sr)
        << ", "
           "failed "
        << pluralise(totals.assertions.failed, "assertion"_sr) << '.';
  } else {
    out << colourImpl->guardColour(Colour::ResultSuccess) << "Passed "
        << bothOrAll(totals.testCases.passed)
        << pluralise(totals.testCases.passed, "test case"_sr) << " with "
...
void
CompactReporter::noMatchingTestCases(StringRef unmatchedSpec)
{
  m_stream << "No test cases matched '" << unmatchedSpec << "'\n";
}

void
CompactReporter::testRunStarting(TestRunInfo const&)
{
...
std::size_t
makeRatio(std::uint64_t number, std::uint64_t total)
{
  const auto ratio =
...
void
ConsoleReporter::noMatchingTestCases(StringRef unmatchedSpec)
{
  m_stream << "No test cases matched '" << unmatchedSpec << "'\n";
}

void
ConsoleReporter::reportInvalidTestSpec(StringRef arg)
{
...
void
ConsoleReporter::benchmarkPreparing(StringRef name)
{
...
void
ConsoleReporter::benchmarkFailed(StringRef error)
{
...
ConsoleReporter::printTotals(Totals const& totals)
{
  if (totals.testCases.total() == 0) {
    ...
    m_stream << m_colour->guardColour(Colour::ResultSuccess)
             << "All tests passed";
    m_stream << " (" << pluralise(totals.assertions.passed, "assertion"_sr)
...
void
ConsoleReporter::printSummaryRow(StringRef label,
  ...
  for (auto col : cols) {
    ...
    if (col.label.empty()) {
      ...
      if (value != "0") {
        ...
        m_stream << m_colour->guardColour(Colour::Warning) << "- none -";
      }
    } else if (value != "0") {
...
struct BySectionInfo
{
  ...
  bool operator()(
    Detail::unique_ptr<CumulativeReporterBase::SectionNode> const& node) const
  {
...
bool
AssertionOrBenchmarkResult::isAssertion() const
{
  return m_assertion.some();
}
bool
AssertionOrBenchmarkResult::isBenchmark() const
{
  return m_benchmark.some();
}

AssertionStats const&
AssertionOrBenchmarkResult::asAssertion() const
{
...
void
CumulativeReporterBase::benchmarkEnded(BenchmarkStats<> const& benchmarkStats)
{
...
void
CumulativeReporterBase::assertionEnded(AssertionStats const& assertionStats)
{
  assert(!m_sectionStack.empty());
...
void
CumulativeReporterBase::testCaseEnded(TestCaseStats const& testCaseStats)
{
...
bool
CumulativeReporterBase::SectionNode::hasAnyAssertions() const
{
...
std::string
fileNameTag(std::vector<Tag> const& tags)
{
...
void
JunitReporter::writeRun(TestRunNode const& testRunNode, double suiteTime)
{
...
namespace Catch {
void
MultiReporter::updatePreferences(IEventListener const& reporterish)
{
...
void
MultiReporter::addListener(IEventListenerPtr&& listener)
{
...
void
MultiReporter::addReporter(IEventListenerPtr&& reporter)
{
...
void
MultiReporter::noMatchingTestCases(StringRef unmatchedSpec)
{
...
void
MultiReporter::fatalErrorEncountered(StringRef error)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::reportInvalidTestSpec(StringRef arg)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::benchmarkPreparing(StringRef name)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::benchmarkStarting(BenchmarkInfo const& benchmarkInfo)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::benchmarkEnded(BenchmarkStats<> const& benchmarkStats)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::benchmarkFailed(StringRef error)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::testRunStarting(TestRunInfo const& testRunInfo)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::testCaseStarting(TestCaseInfo const& testInfo)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::sectionStarting(SectionInfo const& sectionInfo)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::assertionStarting(AssertionInfo const& assertionInfo)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::assertionEnded(AssertionStats const& assertionStats)
{
...
void
MultiReporter::sectionEnded(SectionStats const& sectionStats)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::testCasePartialEnded(TestCaseStats const& testStats,
                                    uint64_t partNumber)
{
...
void
MultiReporter::testCaseEnded(TestCaseStats const& testCaseStats)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::testRunEnded(TestRunStats const& testRunStats)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::skipTest(TestCaseInfo const& testInfo)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::listTests(std::vector<TestCaseHandle> const& tests)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
MultiReporter::listTags(std::vector<TagInfo> const& tags)
{
  for (auto& reporterish : m_reporterLikes) {
...
void
registerReporterImpl(std::string const& name, IReporterFactoryPtr reporterPtr)
{
  CATCH_TRY
  {
...
namespace {
std::string
createRngSeedString(uint32_t seed)
{
  ReusableStringStream sstr;
  sstr << "rng-seed=" << seed;
  return sstr.str();
...
void
SonarQubeReporter::testRunStarting(TestRunInfo const& testRunInfo)
{
...
void
SonarQubeReporter::writeRun(TestRunNode const& runNode)
{
...
void
SonarQubeReporter::writeTestFile(
...
void
SonarQubeReporter::writeTestCase(TestCaseNode const& testCaseNode)
{
  // All test cases have exactly one section - which represents the
  // test case itself. That section may have 0-n nested sections
  assert(testCaseNode.children.size() == 1);
...
void
SonarQubeReporter::writeSection(std::string const& rootName,
                                SectionNode const& sectionNode,
                                bool okToFail)
{
  std::string name = trim(sectionNode.stats.sectionInfo.name);
...
void
SonarQubeReporter::writeAssertions(SectionNode const& sectionNode,
                                   bool okToFail)
{
  for (auto const& assertionOrBenchmark : sectionNode.assertionsAndBenchmarks) {
    if (assertionOrBenchmark.isAssertion()) {
...
void
SonarQubeReporter::writeAssertion(AssertionStats const& stats, bool okToFail)
{
  AssertionResult const& result = stats.assertionResult;
  if (!result.isOk()) {
    std::string elementName;
...
void
StreamingReporterBase::testRunEnded(TestRunStats const&)
{
...
void
TAPReporter::testRunStarting(TestRunInfo const&)
{
  m_stream << "# rng-seed: " << m_config->rngSeed() << '\n';
}

void
TAPReporter::noMatchingTestCases(StringRef unmatchedSpec)
{
  m_stream << "# No test cases matched '" << unmatchedSpec << "'\n";
}

void
TAPReporter::assertionEnded(AssertionStats const& _assertionStats)
{
  ++counter;
...
void
TAPReporter::testRunEnded(TestRunStats const& _testRunStats)
{
...
// subsequent lines
void
printHeaderString(std::ostream& os,
...
void
TeamCityReporter::testRunStarting(TestRunInfo const& runInfo)
{
...
void
TeamCityReporter::testRunEnded(TestRunStats const& runStats)
{
...
void
TeamCityReporter::assertionEnded(AssertionStats const& assertionStats)
{
...
void
TeamCityReporter::testCaseStarting(TestCaseInfo const& testInfo)
{
  m_testTimer.start();
...
void
TeamCityReporter::testCaseEnded(TestCaseStats const& testCaseStats)
{
...
void
TeamCityReporter::printSectionHeader(std::ostream& os)
{
  assert(!m_sectionStack.empty());
...
XmlReporter::testRunStarting(TestRunInfo const& testInfo)
{
  ...
  if (m_config->testSpec().hasFilters())
    m_xml.writeAttribute("filters"_sr,
                         serializeFilters(m_config->getTestsOrTags()));
}
...
void
XmlReporter::benchmarkPreparing(StringRef name)
{
...
void
XmlReporter::benchmarkFailed(StringRef error)
{
...
void
XmlReporter::listReporters(std::vector<ReporterDescription> const& descriptions)
{
...
void
XmlReporter::listListeners(std::vector<ListenerDescription> const& descriptions)
{
...
void
XmlReporter::listTests(std::vector<TestCaseHandle> const& tests)
{
...
void
XmlReporter::listTags(std::vector<TagInfo> const& tags)
{
